...
NCE_W1 = "172.16.2.113"
NCE_W2 = "172.16.2.114"
NCE_IPOutput = "172.16.2.112USOutputPerf.txt"
NCE_W1Output = "172.16.2.113USOutputPerf.txt"
NCE_W2Output = "172.16.2.114USOutputPerf.txt"
DC_IPOutput = "172.16.2.115USOutputPerf.txt"
...
def startDump(IP,deviceCount,fileName):
    try:
        with open(fileName,"a") as fh:
            fh.write("*************************Schedule Performance with %s*******************\n"%str(deviceCount))
...
for input in inputs:
    try:
        ...
        startDump(NCE_IP, NumberOfDevices, NCE_IPOutput)
        startDump(NCE_W1, NumberOfDevices, NCE_W1Output)
        startDump(NCE_W2, NumberOfDevices, NCE_W2Output)
        startDump(DC_IP, NumberOfDevices, DC_IPOutput)

        NumberofCyclePerDevice= (totalDurationmin / int(TimeInterval))+1
        logging.info("------------------------------------------------------------------------------------------------------------------------------------------")
        ...
        while True:
            ...
            time.sleep(topCollectionmin*60)
            countTime=countTime+topCollectionmin
            if countTime >= (totalDurationmin+CoolingTimemin):
                break;

        #time.sleep(totalDurationmin* 60)
